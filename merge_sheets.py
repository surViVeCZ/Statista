...
def merge_excel_sheets(input_file, output_file):
    ...
    for row_idx, row in enumerate(
        tqdm(
            merged_sheet.iter_rows(values_only=True),
            desc="Appending question to options",
        ),
        start=1,
    ):
        if row and isinstance(row[0], str):
            ...
            if match:
                question = match.group(
                    1
                ).strip()  # Capture the question up to the question mark
                merged_sheet.delete_rows(row_idx)
        if question and row and isinstance(row[0], str):
            if any(
                row[0].strip().startswith(stat) for stat in table_stats + [question]
            ):
                pass
            else:  # Append the question to the option in the first column
    ...
    for row_idx, row in enumerate(
        tqdm(
            merged_sheet.iter_rows(values_only=True), desc="Identifying rows to delete"
        ),
        start=1,
    ):
        if (
            ...
            and row[0].strip().lower().startswith("survey name")
        ):
            rows_to_delete.extend(range(row_idx, row_idx + 6))

    # Remove rows from the merged sheet
    for row_idx in tqdm(
        sorted(set(rows_to_delete), reverse=True), desc="Deleting rows"
    ):
    ...
    empty_rows = []
    for row_idx, row in enumerate(
        tqdm(merged_sheet.iter_rows(values_only=True), desc="Identifying empty rows"),
        start=1,
    ):
...
# Example usage
input_file = "short.xlsx"  # Replace with the path to your input Excel file
